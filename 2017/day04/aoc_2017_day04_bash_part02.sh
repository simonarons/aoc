...
while read line; do
	...
	# loop through each elemen in the current array, pipe it through 'fold' to output every character, but one line per character, 
	...
	# I used 'grep -o .' to get the output as 1 char per line, but it seems like 'fold -w1' might shave off ~0.4 seconds previous line is:
	(( ${#psw[@]} == $(for i in ${psw[@]}; do fold -w1 <<<$i|sort|md5sum|cut -f1 -d' '; done|sort -u|wc -l) )) && let sum=sum+1;
	# and that my friends, take 4.5 seconds with my i7-i4790. efficient? hell, yes!
done < $datafile
# and the winner is...
