...
# some check if input yada yada, kill script if input not a file.
if [[ ! -f "$1" ]]; then echo -en "error: input file not detected\nusage: $(basename "$0") [FILE]"; kill -INT $$; fi
...
# so, the datafile layout looks like this:
#
# g	dec	231	if	bfx	>	-10
#
